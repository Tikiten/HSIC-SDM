...
class FullModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.recurrent = net(input_size, 64, 1)
        feat_size = d_out * 1
        feat_size2 = d_out * 1*bands
    ...
    def forward(self, x_spa,x_spe):
        ...
        # print('x_spa:',x_spa.shape) #64,27,27,15
        x_spe = x_spe.squeeze(1)
        x_spa = x_spa[:,:,:,0] #64,27,27

        x_spa = x_spa.unsqueeze(1) #64,1,27,27
        # print('x_spe:',x_spe.shape)
        # print('x_spa:',x_spa.shape)

        x = self.to_tokens(x_spa)
        x = self.to_tokens3(x)

        for transformer, conv in self.stages:
            # print('self.stages[3][0]: ',self.stages[3][0])
        ...
        # print('x_visualize:',x_visualize.shape) # 64,1,27,27
        ###############################################################################
